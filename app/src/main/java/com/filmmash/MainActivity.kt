...
fun NavigationEngine(navController: NavHostController, apiService:ApiService = ApiService()){
    ...
    NavHost(navController = navController, startDestination = "battle"){
        ...
        composable("ratings") {
            val ratingListState = remember { mutableStateOf<RatingList?>(null) }
            ...
            LaunchedEffect(Unit) {
                try {
                    ...
                    apiService.getAllRatings { ratings ->
                        ...
                        ratingList.jsonToRatingList(ratings)
                        ratingListState.value = ratingList
            ...
            when {
                ...
                errorState.value -> {
                    ...
                    ratingListState.value?.let { ratingList ->
                        pages.Ratings(ratingList = ratingList)
                    }
                }
            }
        }
...
fun LoadingIndicator() {
    Column(
        ...
        Text(text = "Loading movies...", fontSize = 16.sp)
    }
}
